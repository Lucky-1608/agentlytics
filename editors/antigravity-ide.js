...
let userDir = path.join(getAppDataPath('Antigravity IDE'), 'User');
if (!fs.existsSync(userDir)) {
  userDir = path.join(getAppDataPath('Antigravity-ide'), 'User');
  if (!fs.existsSync(userDir)) {
    userDir = path.join(getAppDataPath('Antigravity'), 'User');
  }
}
const ANTIGRAVITY_USER_DIR = userDir;
const ANTIGRAVITY_GLOBAL_STORAGE_DB = path.join(ANTIGRAVITY_USER_DIR, 'globalStorage', 'state.vscdb');
const ANTIGRAVITY_BRAIN_DIR = path.join(HOME, '.gemini', 'antigravity-ide', 'brain');
const OFFLINE_TRAJECTORY_SUMMARIES_KEYS = [
  'antigravity-ideUnifiedStateSync.trajectorySummaries',
...
function findLanguageServer() {
  ...
  for (const proc of getProcessList()) {
    ...
    if (!appDirMatch || !appDirMatch[1].includes('antigravity-ide')) continue;
...
const labels = { 'antigravity-ide': 'Antigravity IDE' };
...
function getMCPServers() {
  ...
  let globalConfig = path.join(HOME, '.codeium', 'antigravity-ide', 'mcp_config.json');
  if (!fs.existsSync(globalConfig)) {
    globalConfig = path.join(HOME, '.codeium', 'antigravity', 'mcp_config.json');
  }
  return [
    ...parseMcpConfigFile(globalConfig, { editor: 'antigravity-ide', label: 'Antigravity IDE', scope: 'global' }),
  ];
}
